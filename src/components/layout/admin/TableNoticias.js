...
const TableNoticias = (props) => {
  ...
  useEffect(() => {
    const getNoticas = async () => {
      const token = localStorage.getItem("access_token");

      if (token) {
        try {
          const dataConsulta = await urlAxios.get("/notice", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
          guardarNoticias(dataConsulta.data.data);
        } catch (error) {
  ...
  const eliminarNoticia = async (id) => {
    Swal.fire({
      title: "¿Estas seguro?",
      text: "Ya no se podra recuperar la noticia",
      icon: "warning",
  ...
  const indexUltimaNoticia = paginaActual * grupoPorPagina;
  const indexPrimeraNoticia = indexUltimaNoticia - grupoPorPagina;
  const noticasActuales = noticias.slice(
    indexPrimeraNoticia,
    indexUltimaNoticia
  );

  const totalPaginas = Math.ceil(noticias.length / grupoPorPagina);
    ...
    <>
      <div>
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {noticasActuales.map((noticia, index) => (
              <tr
                key={noticia.id}
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      noticia.description.length > 90
                        ? noticia.description.slice(0, 90) + "..."
                        : noticia.description,
                  }}
                ></td>
                <td className="px-6 py-4">
                  ...
                  <span
                    className={`inline-block  whitespace-nowrap px-1 py-1 rounded-lg text-white ${
                      noticia.status ? "bg-green-500" : "bg-red-500"
                    }`}
                  >
