...
const TableGruposUsuarios = (props) => {
  ...
  useEffect(() => {
    const getGrupos = async () => {
      ...
      if (token) {
        try {
          const dataConsulta = await urlAxios.get("/groups/grupo-usuario", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
          guardarGrupos(dataConsulta.data);
        } catch (error) {
...
export default TableGruposUsuarios;
